...
from MUnet_and_Denoiser import DocDiff, EMA
from util.diffusionSample import GaussianDiffusion
from util.dpm_solver_pytorch import NoiseScheduleVP, model_wrapper, DPM_Solver
import torch
...
import copy
from util.sobel import Laplacian
...
class Trainer:
    def __init__(self, config):
        ...
        from util.docdata import DocData
        if self.mode == 1:
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for img, gt, name in tq:
                ...
                finalImgs = finalImgs[:, :, :h, :w]
                init_predict = init_predict[:, :, :h, :w]
                img = img[:, :, :h, :w]
                ...
                img_save = torch.cat((img, gt, init_predict.cpu(), min_max(sampledImgs[:, :, :h, :w].cpu()), finalImgs.cpu()), dim=3)
    ...
    def train(self):
        ...
        while iteration < self.iteration_max:
            ...
            for img, gt, name in tq:
                ...
                init_predict, noise_pred, noisy_image, noise_ref, init_predict_list = self.network(gt.to(self.device), img.to(self.device), t, self.diffusion)
                ...
                # loss = 0.75 * (ddpm_loss + self.beta_loss * (pixel_loss) / self.num_timesteps) + loss_content + 0.1 * loss_fft
                ...
                if self.high_low_freq == 'True':
                    ...
                    tq.set_postfix(loss=loss.item(), ddpm_loss=ddpm_loss.item(), pixel_loss=pixel_loss.item(), psnr=psnr_loss)
                if iteration % 500 == 0:
