...
class DocData10(Dataset):
    def __init__(self, path_img, path_gt, loadSize, mode=1):
        ...
        # 确保两个列表的长度相同
        assert len(self.data_gt) == len(self.data_img), "两个路径下的图片数量不一致"

        # 如果图片数量大于等于 10 张，则随机抽取 10 张
        if len(self.data_gt) >= 10:
            # 随机抽取 10 个索引
            random_indices = random.sample(range(len(self.data_gt)), 10)

            # 根据索引抽取对应的图片名称
            self.data_gt = [self.data_gt[i] for i in random_indices]
            self.data_img = [self.data_img[i] for i in random_indices]
