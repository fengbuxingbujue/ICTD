...
def extract(v, t, x_shape):
    ...
    在指定的时间步长提取一些系数，然后重塑为[batch_size,1,1,1,1,…]作广播之用
    """
    device = t.device
...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_t, cond_, t):
        ...
        # 在KL计算中只使用log_variance
        var = torch.cat([self.posterior_var[1:2], self.betas[1:]])
        # 注意上面的切片方式，写法是 posterior_var[1:2]，而不是 posterior_var[1]
        # 原因是第一种方法返回的是长度为1的切片，数据类型保持不变
        # 而第二种方法只会返回当前位置上的元素，数据类型会发生变化
        # 举例来说：
        # x = [0, 1, 2, 3, 4]; x[1:2] = [1]; x[1] = 1
        # var = self.betas
