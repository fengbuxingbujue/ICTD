...
from DocDiff_MUnet_and_Denoiser7_27 import DocDiff, EMA
from diffusionSample import GaussianDiffusion
...
def init__result_Dir():
    work_dir = os.path.join(r'./datasets/GoPro', 'train')
    max_model = 0
...
class Trainer:
    def __init__(self, config):
        ...
        from docdata10 import DocData10 as DocData
        if self.mode == 1:
    ...
    def test(self):
        ...
        def crop_concat_back(img, prediction, size=128):
            ...
            for i in range(shape[2] // size + 1):
                for j in range(shape[3] // size + 1):
                    if j == 0:
                        crop = prediction[
                               (i * (shape[3] // size + 1) + j) * shape[0]:(i * (shape[3] // size + 1) + j + 1) * shape[
                                   0], :, :, :]
                    else:
                        crop = torch.cat((crop, prediction[(i * (shape[3] // size + 1) + j) * shape[0]:(i * (
                                    shape[3] // size + 1) + j + 1) * shape[0], :, :, :]), dim=3)
                if i == 0:
                    crop_concat = crop
                else:
        ...
        with torch.no_grad():
            ...
            '''
            self.network.init_predictor.apply(convert_module_to_f16)
            self.network.denoiser.apply(convert_module_to_f16)
            for name, param in self.network.init_predictor.named_parameters():
            ...
            for img, gt, name in tq:
                ...
                init_predict = self.network.init_predictor(img.to(self.device), 0)[-1]
                ...
                '''
                img = img.float()
                init_predict = init_predict.float()
                finalImgs = finalImgs.float()
                '''

                if self.findmax:
                    final_psnr_value = calculate_psnr(gt, finalImgs.cpu())
                    init_psnr_value = calculate_psnr(gt, init_predict.cpu())
                    if final_psnr_value < init_psnr_value:
                ...
                psnr_value = calculate_psnr(gt, finalImgs.cpu())
                psnr_values.append(psnr_value)
                with open('psnr_values.txt', 'a') as f:
                    f.write(f'{name[0]}: {psnr_value}\n')
                print(f"{name[0]} PSNR = {psnr_value}")

            average_psnr = np.mean(psnr_values)
            print(average_psnr)
            print(init_flag)
            with open('psnr_values.txt', 'a') as f:
                f.write(f'最终的平均值为: {average_psnr}\n')
    ...
    def train(self):
        ...
        while iteration < self.iteration_max:
            ...
            for img, gt, name in tq:
                ...
                init_predict, noise_pred, noisy_image, noise_ref, init_predict_list = self.network(gt.to(self.device),
                                                                                                   img.to(self.device),
                                                                                                   t, self.diffusion)
                gt_img = gt.to(self.device)
                gt_img2 = F.interpolate(gt.to(self.device), scale_factor=0.5, mode='bilinear')
                gt_img4 = F.interpolate(gt.to(self.device), scale_factor=0.25, mode='bilinear')
                l1 = criterion(init_predict_list[0], gt_img4)
                l2 = criterion(init_predict_list[1], gt_img2)
                l3 = criterion(init_predict_list[2], gt_img)
                loss_content = l1 + l2 + l3
                gt_img_fft = torch.fft.rfft2(gt_img, dim=(-2, -1), norm=None)
                gt_img2_fft = torch.fft.rfft2(gt_img2, dim=(-2, -1), norm=None)
                gt_img4_fft = torch.fft.rfft2(gt_img4, dim=(-2, -1), norm=None)
                init_fft = torch.fft.rfft2(init_predict_list[2], dim=(-2, -1), norm=None)
                init2_fft = torch.fft.rfft2(init_predict_list[1], dim=(-2, -1), norm=None)
                init4_fft = torch.fft.rfft2(init_predict_list[0], dim=(-2, -1), norm=None)
                f1 = criterion(gt_img_fft, init_fft)
                f2 = criterion(gt_img2_fft, init2_fft)
                f3 = criterion(gt_img4_fft, init4_fft)
                loss_fft = f1 + f2 + f3

                if self.pre_ori == 'True':
                    if self.high_low_freq == 'True':
                        residual_high = self.high_filter(gt.to(self.device) - init_predict)
                        ddpm_loss = 2 * self.loss(self.high_filter(noise_pred), residual_high) + self.loss(noise_pred,
                                                                                                           gt.to(
                                                                                                               self.device) - init_predict)
                    else:
                ...
                if self.high_low_freq == 'True':
                    ...
                    low_freq_loss = self.loss(init_predict - self.high_filter(init_predict),
                                              gt.to(self.device) - self.high_filter(gt.to(self.device)))
                    pixel_loss = low_high_loss + 2 * low_freq_loss
                else:
                    pixel_loss = self.loss(init_predict, gt.to(self.device))

                psnr_loss = calculate_psnr(gt.to(self.device), noise_pred + init_predict)

                loss = 0.75 * (ddpm_loss + self.beta_loss * (
                    pixel_loss) / self.num_timesteps) + loss_content + 0.1 * loss_fft
                # loss = ddpm_loss + self.beta_loss * (pixel_loss) / self.num_timesteps

                loss.backward()
                optimizer.step()
                if self.high_low_freq == 'True':
                    tq.set_postfix(loss=loss.item(), high_freq_ddpm_loss=ddpm_loss.item(),
                                   low_freq_pixel_loss=low_freq_loss.item(),
                                   pixel_loss=low_high_loss.item(), psnr=psnr_loss)
                else:
                ...
                if iteration % 500 == 0:
                    ...
                    if self.pre_ori == 'True':
                        if self.high_low_freq == 'True':
                            img_save = torch.cat(
                                [img, gt, init_predict.cpu(), noise_pred.cpu() + self.high_filter(init_predict).cpu(),
                                 noise_pred.cpu() + init_predict.cpu()], dim=3)
                        else:
                            img_save = torch.cat([img, gt, init_predict.cpu(), noise_pred.cpu() + init_predict.cpu()],
                                                 dim=3)
                    save_image(img_save, os.path.join(
